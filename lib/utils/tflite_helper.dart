...
class TFLiteHelper {
  ...
  static Future<tfl.Interpreter?> loadModelFromAssets(String modelPath) async {
    try {
      ...
      return await tfl.Interpreter.fromBuffer(modelData.buffer.asUint8List());
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> runModelOnImage({
    ...
    try {
      ...
      if (inputTensor.type != tfl.TensorType.float32) {
        throw Exception('Model input tensor is not Float32. This helper is not compatible.');
      }
      final expectedSizeInBytes = inputShape.reduce((a, b) => a * b) * 4;

      if (input.lengthInBytes != expectedSizeInBytes) {
        throw Exception('Input tensor size mismatch. Expected $expectedSizeInBytes bytes, but got ${input.lengthInBytes} bytes.');
      }
